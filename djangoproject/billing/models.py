...
class BillingProfile(models.Model):
	...
	customer_id = models.CharField(max_length=120, null=True, blank=True) # customer_id in stripe or braintee 
	...
	def get_payment_method_url():
		return reverse("billing-payement-method")
...
def billing_profile_created_receiver(sender, instance, *args, **kwargs):
	if not instance.customer_id and instance.email:
		print("Actual API Request Send to stripe/braintee")
...
class ChargeManager(models.Manager):
	def do(self, billing_profile, order_obj, card=None):
		...
		c = stripe.Charge.create(
			amount = int(order_obj.total * 100), #29.19 ---> 2919
			currency = "usd",
